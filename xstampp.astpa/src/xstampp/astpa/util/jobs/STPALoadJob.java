...
//			}
				FileInputStream reader= new FileInputStream(this.getFile());
...
//			Reader stream= new StringReader(buffer.toString()); 
			Source xmlFile = new StreamSource(reader,getFile().toURI().toASCIIString());
...
//			StringReader stringReader=new StringReader(buffer.toString());
			reader= new FileInputStream(this.getFile());
			this.setController((IDataModel) um.unmarshal(reader));
			reader.close();
