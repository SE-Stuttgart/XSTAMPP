...
public class LinkingView extends StandartEditorPart implements IViewBase {
	...
	private void updateSelectionTable() {
		if (this.accidentMode) {
			this.selectionTableViewer.setInput(this.dataInterface
					.getAllAccidents());
		} else {
			this.selectionTableViewer.setInput(this.dataInterface
					.getAllHazards());
	...
	private void updateAvailableTable() {
		if (this.selectedId != null) {
			if (this.accidentMode) {
				java.util.List<ITableModel> linked = this.dataInterface
						.getLinkedHazards(this.selectedId);
				...
				java.util.List<ITableModel> linked = this.dataInterface
						.getLinkedAccidents(this.selectedId);
	...
	private void updateCurrentTable() {
		if (this.selectedId != null) {
			if (this.accidentMode) {
				this.currentTableViewer.setInput(this.dataInterface
						.getLinkedHazards(this.selectedId));
			} else {
				this.currentTableViewer.setInput(this.dataInterface
						.getLinkedAccidents(this.selectedId));
	...
	private void updateSelection() {
		...
		if (selection.size() > 0) {
			this.selectedId = ((ITableModel) selection.getFirstElement())
					.getId();
		} else {
	...
	private Object[] getSelectedAvailable() {
		if (!LinkingView.this.availableTableViewer.getSelection().isEmpty()) {
			return ((IStructuredSelection) LinkingView.this.availableTableViewer
	...
	private Object[] getSelectedCurrent() {
		if (!LinkingView.this.currentTableViewer.getSelection().isEmpty()) {
			return ((StructuredSelection) LinkingView.this.currentTableViewer
					.getSelection()).toArray();
		}
	...
	private void removelAllCurrentLinks() {
		...
		if (LinkingView.this.accidentMode) {
			...
			if ((elementId != null) && (current != null)) {
				...
				while (iter.hasNext()) {
					this.dataInterface.deleteLink(elementId, iter.next()
							.getId());
				}
			}
			...
			if ((elementId != null) && (current != null)) {
				Iterator<ITableModel> iter = current.iterator();
				while (iter.hasNext()) {
					this.dataInterface.deleteLink(iter.next().getId(),
							elementId);
				}
			}
		}
		...
		if (selected != null) {
			...
			for (int i = 0; i < accidentIDs.size(); i++) {
				for (int j = 0; j < hazardIDs.size(); j++) {
					this.dataInterface.deleteLink(accidentIDs.get(i),
							hazardIDs.get(j));
				}
			}
	...
	public void createPartControl(Composite parent) {
		...
		selectionArea.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true,
				true, 1, 1));
		selectionArea.setLayout(new GridLayout(1, false));

		this.lblSelected = new Label(selectionArea, SWT.NONE);
		this.lblSelected.setLayoutData(new GridData(SWT.FILL, SWT.TOP, false,
				false, 1, 1));
		...
		tableLayoutComposite.setLayoutData(new GridData(SWT.FILL, SWT.FILL,
				true, true));
		...
		TableViewerColumn selectionTableIdColumn = new TableViewerColumn(
				this.selectionTableViewer, SWT.NONE);
		...
		TableViewerColumn selectionTableTitleColumn = new TableViewerColumn(
				this.selectionTableViewer, SWT.NONE);
		selectionTableTitleColumn.setLabelProvider(titleLabelProvider);
		TableColumn selectionShortDescriptionColumn = selectionTableTitleColumn
				.getColumn();
		selectionShortDescriptionColumn.setText(Messages.Title);

		Composite modeButtonArea = new Composite(selectionArea, SWT.NONE);
		modeButtonArea.setLayoutData(new GridData(SWT.FILL, SWT.FILL, false,
				false, 1, 1));
		...
		this.modeButton.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				LinkingView.this
						.setAccidentMode(!LinkingView.this.accidentMode);
			}
		});
		this.modeButton.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true,
				false, 1, 1));
		...
		editingForm.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, true,
				1, 1));
		editingForm.setLayout(new GridLayout(1, true));

		this.lblAvailableForLinking = new Label(editingForm, SWT.NONE);
		this.lblAvailableForLinking.setLayoutData(new GridData(SWT.FILL,
				SWT.TOP, false, false, 1, 1));
		this.lblAvailableForLinking.setFont(labelFont);
		final Composite availableTableLayoutComposite = new Composite(
				editingForm, SWT.BORDER);
		availableTableLayoutComposite.setLayoutData(new GridData(SWT.FILL,
				SWT.TOP, true, true));
		availableTableLayoutComposite.setLayout(tableLayout);

		this.availableTableViewer = new TableViewer(
				availableTableLayoutComposite, SWT.FULL_SELECTION | SWT.MULTI
						| SWT.V_SCROLL);
		...
		TableViewerColumn availableTableIdColumn = new TableViewerColumn(
				this.availableTableViewer, SWT.NONE);
		availableTableIdColumn.setLabelProvider(idLabelProvider);

		TableViewerColumn availableTableTitleColumn = new TableViewerColumn(
				this.availableTableViewer, SWT.NONE);
		...
		buttonArea.setLayoutData(new GridData(SWT.CENTER, SWT.FILL, true,
				false, 1, 1));
		buttonArea.setLayout(new GridLayout(LinkingView.BUTTON_AREA_COLUMNS,
				true));
		...
		addAllButton.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true,
				false, 1, 1));
		...
		removeAllButton.setLayoutData(new GridData(SWT.FILL, SWT.FILL, false,
				false, 1, 1));
		...
		this.lblCurrentlyLinked.setLayoutData(new GridData(SWT.FILL, SWT.TOP,
				false, false, 1, 1));
		this.lblCurrentlyLinked.setFont(labelFont);
		final Composite currentTableLayoutComposite = new Composite(
				editingForm, SWT.BORDER);
		currentTableLayoutComposite.setLayoutData(new GridData(SWT.FILL,
				SWT.TOP, true, true));
		...
		TableViewerColumn currentViewerColumnId = new TableViewerColumn(
		...
		TableViewerColumn currentViewerColumnShortDescription = new TableViewerColumn(
				this.currentTableViewer, SWT.NONE);
		currentViewerColumnShortDescription
				.setLabelProvider(titleLabelProvider);

		TableColumn currentColumnTitle = currentViewerColumnShortDescription
				.getColumn();
		...
		TableColumn availableColumnTitle = availableTableTitleColumn
				.getColumn();
		...
		tableLayout.setColumnData(selectionIdColumn, new ColumnWeightData(
				LinkingView.COLUMN_ID_WEIGHT_X,
				LinkingView.COLUMN_ID_WEIGHT1_Y, false));
		tableLayout.setColumnData(selectionShortDescriptionColumn,
				new ColumnWeightData(LinkingView.COLUMN_TITLE_WEIGHT_X,
						LinkingView.COLUMN_TITLE_WEIGHT_Y, false));
		tableLayout.setColumnData(currentColumnId, new ColumnWeightData(
				LinkingView.COLUMN_ID_WEIGHT_X,
				LinkingView.COLUMN_ID_WEIGHT1_Y, false));
		tableLayout.setColumnData(currentColumnTitle, new ColumnWeightData(
				LinkingView.COLUMN_TITLE_WEIGHT_X,
				LinkingView.COLUMN_TITLE_WEIGHT_Y, false));
	...
	private void updateMode() {
		if (this.accidentMode) {
			...
			this.lblAvailableForLinking
					.setText(Messages.HazardsAvailableForLinking);
			...
			this.lblAvailableForLinking
					.setText(Messages.AccidentsAvailableForLinking);
