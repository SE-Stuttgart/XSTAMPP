...
public class GridCellEditor extends GridCellComposite {
	...
	public GridCellEditor(GridWrapper gridWrapper, String initialText) {
		...
		this.description.addModifyListener(new ModifyListener() {
			...
			public void modifyText(ModifyEvent e) {
				GridCellEditor.this.currentText = GridCellEditor.this.description
						.getText();
		...
		this.description.addListener(SWT.FocusOut, new Listener() {
			...
			public void handleEvent(Event event) {
				GridCellEditor.this
						.onTextChanged(GridCellEditor.this.currentText);
		...
		this.description.addListener(SWT.KeyUp, new Listener() {
			...
			public void handleEvent(Event event) {
				if (event.character == SWT.CR) {
					GridCellEditor.this.description
	...
	public void paint(GridCellRenderer renderer, GC gc,
		...
		if (this.hasFocus) {
			...
			this.description.setBackground(this
					.getBackgroundColor(renderer, gc));
		}
	...
	public int getPreferredHeight() {
		...
		if ((textSize == 0) && (this.grid.getGrid().getColumnCount() > 0)) {
			this.description.setSize(new Point((this.grid.getGrid()
					.getColumn(1).getWidth() - 33), 0));
		}
		int preferredHeight = (this.description.getLineHeight() * this.description
				.getLineCount()) + AbstractGridCell.DEFAULT_CELL_HEIGHT;
