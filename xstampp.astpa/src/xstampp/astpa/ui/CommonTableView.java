...
public abstract class CommonTableView<T extends IDataModel> extends StandartEditorPart {
		...
		public void selectionChanged(SelectionChangedEvent event) {
			...
			if (event.getSelection() instanceof IStructuredSelection) {
				...
				if (selection.getFirstElement() instanceof ATableModel) {
					...
					getDescriptionWidget().setText(((ATableModel) selection.getFirstElement()).getDescription());
	 ...
	 * 
	 * @param restrict
	 *            a boolean that indicates whether this table can only be edited by
	 *            a user possesing the access rights in a user system.
	 */
	...
	public void createCommonTableView(Composite parent, String tableHeader) {
		...
		this.descriptionWidget = new Text(leftSash, SWT.MULTI | SWT.BORDER | SWT.V_SCROLL | SWT.WRAP | SWT.SEARCH);
		...
		this.getTableViewer().getTable().addListener(SWT.KeyDown, new Listener() {
			...
			public void handleEvent(Event event) {
				if ((event.type == SWT.KeyDown) && (event.keyCode == SWT.CR)
					...
					getTitleColumn().getViewer().editElement(getTableViewer().getElementAt(indexFirstSelected), 1);
				}
			}
		});
	...
	protected boolean canEdit(ATableModel entryId, AccessRights level) {
		if (getDataInterface() instanceof IUserProject) {
			...
			if (user != null && user.getUserId().equals(entryId.getCreatedBy())) {
				...
				return ((IUserProject) getDataInterface()).getUserSystem().checkAccess(entryId.getId(), level);
			}
		}
		return true;
	}
