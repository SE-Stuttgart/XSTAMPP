...
public class ChooseWorkLocation extends TitleAreaDialog {
	 ...
	 * resentWs Array by splitting the WS_RECENT String in the Preference store
	 * by the SEPERATOR
	 * 
	 * @author Lukas Balzer
	 * 
	...
	public ChooseWorkLocation(Shell parentShell) {
		...
		if (recentLocations != null) {
			String[] pathArray = recentLocations
					.split(ChooseWorkLocation.SEPERATOR);
	...
	protected Control createDialogArea(Composite parent) {
			...
			public void handleEvent(Event event) {
				...
				if ((pick == null)
						&& (ChooseWorkLocation.this.workspacePathCombo
								.getText().length() == 0)) {
					ChooseWorkLocation.this.setMessage(Messages.IlegalPath,
		...
		if (!this.recentLocs.isEmpty()) {
			this.workspacePathCombo.setItems(this.recentLocs
	...
	private boolean addRecentWS(String path) {
		...
		for (int i = 1; i < ChooseWorkLocation.STACK; i++) {
			if ((this.recentLocs.size() > i)
					&& path.equals(this.recentLocs.get(i))) {
