...
public class UpdateJob extends Job {
	...
	private static final String REPOSITORY_LOC = System.getProperty("UpdateHandler.Repo", //$NON-NLS-1$
		Activator.getDefault().getPreferenceStore().getDefaultString(IPreferenceConstants.UPDATE_LINK));
	 ...
	 * @param startUp true, if this is the check at the startup
	 */
	public UpdateJob(String name, IProvisioningAgent agent, Shell parent, ViewContainer viewContainer, boolean startUp) {
		super(name);
		this.agent = agent;
		this.parent = parent;
			...
			Display.getDefault().syncExec(new Runnable() {
				...
				public void run() {
					...
					if (possibleUpdates.length > 0) {
						UpdateJob.this.doInstall =
							MessageDialog.openQuestion(UpdateJob.this.parent, Messages.ReallyInstallUpdates, updates);
	...
	public void done(final IJobChangeEvent event) {
		if (event.getResult().isOK()) {
			Display.getDefault().syncExec(new Runnable() {
				...
				public void run() {
					boolean restart =
						MessageDialog.openQuestion(UpdateJobChangeAdapter.this.parent,
					...
					if (UpdateJobChangeAdapter.this.viewContainer.getUnsavedChanges()) {
						restart =
							restart
								&& !MessageDialog.openQuestion(UpdateJobChangeAdapter.this.parent, Messages.ASTPA,
