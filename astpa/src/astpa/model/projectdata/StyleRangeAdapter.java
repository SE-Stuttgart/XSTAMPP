...
package astpa.model.projectdata;
...
public class StyleRangeAdapter extends XmlAdapter<String, StyleRange> {
	...
	public String marshal(StyleRange sr) throws Exception {
		...
		if (sr.foreground == null) {
			sb.append(sr.foreground + StyleRangeAdapter.DELIMITER + sr.foreground + StyleRangeAdapter.DELIMITER
				+ sr.foreground + StyleRangeAdapter.DELIMITER);
		} else {
			sb.append(sr.foreground.getRed() + StyleRangeAdapter.DELIMITER + sr.foreground.getGreen()
				+ StyleRangeAdapter.DELIMITER + sr.foreground.getBlue() + StyleRangeAdapter.DELIMITER);
		}
		// Background color
		if (sr.background == null) {
			sb.append(sr.background + StyleRangeAdapter.DELIMITER + sr.background + StyleRangeAdapter.DELIMITER
				+ sr.background + StyleRangeAdapter.DELIMITER);
		} else {
			sb.append(sr.background.getRed() + StyleRangeAdapter.DELIMITER + sr.background.getGreen()
				+ StyleRangeAdapter.DELIMITER + sr.background.getBlue() + StyleRangeAdapter.DELIMITER);
		}
		// Font
		sb.append(sr.fontStyle + StyleRangeAdapter.DELIMITER);
		if (sr.font == null) {
	...
	public StyleRange unmarshal(String s) throws Exception {
		...
		if (data[6].equals("null")) { //$NON-NLS-1$
			...
			sr.foreground =
				new Color(Display.getCurrent(), Integer.parseInt(data[6]), Integer.parseInt(data[7]),
		...
		if (data[9].equals("null")) { //$NON-NLS-1$
			...
			sr.background =
				new Color(Display.getCurrent(), Integer.parseInt(data[9]), Integer.parseInt(data[10]),
