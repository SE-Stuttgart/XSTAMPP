...
import astpa.model.controlaction.interfaces.IUCAHazLink;
...
public class ControlActionController {
	...
	public UUID addControlAction(String title, String description) {
		ControlAction controlAction = new ControlAction(title, description, this.controlActions.size() + 1);
	 ...
	 * 
	 * @param controlActionId control action's ID
	 * 
	 ...
	 * 
	 * @param controlActionId id of the control action
	 ...
	 * 
	 * @param controlActionId the id of the control action
	 * @param description the description of the new unsafe control action
	 * @param unsafeControlActionType the type of the new unsafe control action
	...
	public UUID addUnsafeControlAction(UUID controlActionId, String description,
		...
		return controlAction.addUnsafeControlAction(description, unsafeControlActionType);
	}
	
	/**
	 * Searches the unsafe control action and removes it
	 * 
	 * @param unsafeControlActionId the id of the unsafe control action to
	 *            delete
		...
		for (ControlAction controlAction : this.controlActions) {
			for (IUnsafeControlAction uca : controlAction.getUnsafeControlActions()) {
				if (uca.getId().equals(unsafeControlActionId)) {
					...
					return controlAction.removeUnsafeControlAction(unsafeControlActionId);
				}
			}
		}
		return false;
	 ...
	 * 
	 * @param unsafeControlActionId the id of the unsafe control action
	 ...
	 * 
	 * @param unsafeControlActionId the unsafe control action of which a link
	 *            will be deleted.
	 ...
	 * @author Fabian Toth
	 * 
	 * @param id the id of one part of the link
	 ...
	 * 
	 * @param unsafeControlActionId the uca's id.
	 ...
	 * 
	 * @param unsafeControlActionId the id of the unsafe control action
	 * @param safetyConstraintDescription the text of the corresponding safety
	 *            constraint
	...
	public List<ISafetyConstraint> getCorrespondingSafetyConstraints() {
		...
		for (ICorrespondingUnsafeControlAction unsafeControlAction : this.getAllUnsafeControlActions()) {
	...
	public void prepareForExport(HazAccController hazAccController) {
		for (ControlAction controlAction : this.controlActions) {
			for (UnsafeControlAction unsafeControlAction : controlAction.getInternalUnsafeControlActions()) {
				...
				for (IUCAHazLink link : this.getLinksOfUCA(unsafeControlAction.getId())) {
					linkedHazards.add(hazAccController.getHazard(link.getHazardId()));
				}
				Collections.sort(linkedHazards);
				if (linkedHazards.size() == 0) {
					unsafeControlAction.setLinks(Messages.ControlActionController_NotHazardous);
				} else {
