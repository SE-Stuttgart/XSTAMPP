...
public class ComponentRenameCommand extends ControlStructureAbstractCommand {
	...
	@Override
	public void execute() {
		this.getDataModel().changeComponentText(this.model.getId(), this.newName);
		
		if (!this.model.getLayout(this.getStepID().equals(CSEditor.ID)).contains(this.newLayout)) {
			this.getDataModel().changeComponentLayout(this.model.getId(), this.newLayout,
	...
	@Override
	public void undo() {
		this.getDataModel().changeComponentText(this.model.getId(), this.oldName);
	 ...
	 * @author Lukas Balzer
	 * 
	 * @param newName the text to which the "oldName" is set
	 * 
	 */
	 ...
	 * @author Lukas Balzer
	 * 
	 * @param model The model which stored the text
	 */
