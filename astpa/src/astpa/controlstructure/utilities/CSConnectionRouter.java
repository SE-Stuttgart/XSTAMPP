...
public class CSConnectionRouter extends AbstractRouter {
	 ...
	 * @author Lukas Balzer
	 * 
	 * @param ownerPart the Part in which the connections should be calculated
	 *            in
	 */
	...
	public void route(Connection connection) {
		...
		if (connection.getTargetAnchor() instanceof CSAnchor && connection.getSourceAnchor() instanceof CSAnchor
			&& insectOwner(connection)) {
	 ...
	 * @author Lukas Balzer
	 * 
	 * @param connection the connection which should be idealized
	 */
	public void idealize(Connection connection) {
		...
		Point endOrientation = ((CSAnchor) connection.getTargetAnchor()).getOrientation();
		
		Point startOrientation = ((CSAnchor) connection.getSourceAnchor()).getOrientation();
		...
		} while (this.csMatrix[start.x][start.y] == SPACE_RESERVED);
		
		if (connection.getTargetAnchor() instanceof CSAnchor) {
			do {
				end.translate(endOrientation);
			} while (this.csMatrix[end.x][end.y] == SPACE_RESERVED);
			
		}
		
		tempPoints = idealPath(tempPoints, start.x, start.y, 0, startOrientation, end, false, false);
	 ...
	 * @param dist the current distance of the path
	 * @param direction the direction in which the algorithm moves, this
	 *            information is needed to calculate the distance
	 * @param end the end point
	 * @param forceY
	 * @param forceX
	 * @return
	 */
	private PointList idealPath(PointList list, int x, int y, int distance, Point direction, Point end, boolean forceY,
		boolean forceX) {
		...
		} else if (this.csMatrix[x][y] == SPACE_RESERVED) {
			return null;
		} else if (direction.equals(STILL)) {
			return null;
		} else if (this.csMatrix[x][y] < dist) {
			idealPoints.addPoint(RESERVED_POINT);
		} else {
			this.csMatrix[x][y] = dist;
			PointList xList = calcXlists(direction, x, y);
			PointList yList = calcYlists(direction, x, y);
			PointList yTemp = null;
			PointList xTemp = null;
			
			// if the algorithm cannot move in y direction, the x-direction is
			// forced
			changeY = signOf(end.y - y, yForced, direction.y);
			xForced = (this.csMatrix[x][y + changeY] == SPACE_RESERVED);
			
			// if the algorithm cannot move in x direction, the y-direction is
			// forced
			changeX = signOf(end.x - x, xForced, direction.x);
			yForced = (this.csMatrix[x + changeX][y] == SPACE_RESERVED);
			
			xTemp = idealPath(xList, x + changeX, y, dist, xChange(changeX, direction.x), end, yForced, xForced);
			
			yTemp = idealPath(yList, x, y + changeY, dist, yChange(changeY, direction.y), end, yForced, xForced);
			if (foundEnd(yTemp, end)) {
				idealPoints.addAll(yTemp);
				return idealPoints;
			}
			if (isRedundant(yTemp, xTemp)) {
				return null;
			}
			
			// if the algorithm does not find a path in x- or y-direction
			// xTemp is calculated again with forced attributes
			if (isNull(yTemp, xTemp)) {
				changeX = absMax(direction.x);
				xTemp = idealPath(xList, x + changeX, y, dist, xChange(changeX, direction.x), end, false, true);
				
				// if xTemp is still null yTemp is recalculated
				if (xTemp == null) {
					changeY = absMax(direction.y);
					yTemp = idealPath(xList, x, y + changeY, dist, yChange(changeY, direction.y), end, true, false);
				}
				
			}
			if (!isNull(yTemp, xTemp)) {
				idealPoints.addAll(choose(xTemp, yTemp));
			}
	...
	private PointList calcXlists(Point currentDir, int x, int y) {
		...
		if (currentDir.equals(UP) || currentDir.equals(DOWN)) {
	...
	private PointList calcYlists(Point currentDir, int x, int y) {
		PointList xList = new PointList();
		if (currentDir.equals(LEFT) || currentDir.equals(RIGHT)) {
			xList.addPoint(new Point(x, y));
		}
		return xList;
	}
	...
	private boolean isRedundant(PointList a, PointList b) {
		...
		if (a != null && a.getLastPoint().equals(RESERVED_POINT)) {
			aRedundant = true;
		}
		if (b != null && b.getLastPoint().equals(RESERVED_POINT)) {
			bRedundant = true;
	 ...
	 * @author Lukas Balzer
	 * 
	 * @param delta the int the leading sign is needed
	 * @param force if the function should return 1
	 * @param movement says if the algorithm moves in positive or negative
	 *            direction
	...
	private boolean fillMatrix() {
		...
		int[][] childMap = raster(xThreshold, yThreshold);
		
		Iterator<IRectangleComponent> childIterator = this.owner.getModelChildren().iterator();
		int x;
		int y;
		while (childIterator.hasNext()) {
			Rectangle childRect =
				childIterator.next().getLayout(
					this.owner.getViewer().getProperty(CSAbstractEditor.STEP_EDITOR).toString().equals(CSEditor.ID));
			
			// iterates over all points of the child and sets them to 0
			// in the childMap
			for (int xOffset = -INTERVALL; xOffset < childRect.width + INTERVALL; xOffset++) {
				x = xOffset + childRect.x;
				if (x < 0 || x > xThreshold) {
					x = 0;
				}
				for (int yOffset = -INTERVALL; yOffset < childRect.height + INTERVALL; yOffset++) {
					y = yOffset + childRect.y;
					if (y < 0 || y > yThreshold) {
						y = 0;
					}
					try {
						childMap[x][y] = SPACE_RESERVED;
	...
	private boolean insectOwner(Connection conn) {
		...
		while (childIterator.hasNext()) {
			...
			childRect.shrink(INTERSECTION_THRESHOLD);
	 ...
	 * @author Lukas Balzer
	 * 
	 * @param newMatrix a 2D array of integers which stores all pixels Whether
	 *            there is an component or not
	 */
