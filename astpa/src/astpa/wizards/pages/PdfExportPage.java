...
public class PdfExportPage extends AbstractExportPage implements ModifyListener{
	...
	public void createControl(Composite parent) {
		...
		//----Create the background color chooser composite---------------------------
		...
		data=new FormData();
		data.top = new FormAttachment(this.logoComposite,COMPONENT_OFFSET);
		data.height = 25;
		this.bgChooser.setLayoutData(data);
		this.bgChooser.setVisible(true);
		this.bgChooser.addColorChangeListener(this);

		//----Create the foreground color chooser composite---------------------------
		...
		this.pathComposite= new PathComposite(new String[]{"*.pdf"}, //$NON-NLS-1$
		...
		this.pathComposite.setVisible(true);
		
		//----Creates a Composite for the switch which turns the decoration of the control structure on/off
		Composite decoSwitchComposite = new Composite(this.container, SWT.NONE);
		data= new FormData();
		data.top= new FormAttachment(this.pathComposite,COMPONENT_OFFSET);
		...
		//----Creates a Composite for a Canvas which provides a preview of the projectname with the chosen fore-/background colors
	...
	public void setExportPath(String path) {
		this.pathComposite.setText(path);
