...
public class HazardsView extends CommonTableView {
	 ...
	 * 
	 */
	public static final String ID = "astpa.ui.acchaz.hazardsviewpart"; //$NON-NLS-1$
	...
	public void createPartControl(Composite parent) {
		...
		linksColumn.setLabelProvider(new ColumnLabelProvider() {
			...
			public String getText(Object element) {
				...
				List<ITableModel> links = HazardsView.this.dataInterface.getLinkedAccidents(((Hazard) element).getId());
				if (!(links == null)) {
			...
			public void keyReleased(final KeyEvent e) {
				if ((e.keyCode == SWT.DEL) || ((e.stateMask == SWT.COMMAND) && (e.keyCode == SWT.BS))) {
					IStructuredSelection selection =
						(IStructuredSelection) HazardsView.this.getTableViewer().getSelection();
	...
	public void deleteItems() {
		...
		if (selection.size() == 1) {
			...
			String confirmation = Messages.PleaseConfirmTheDeletionOfTheFollowingHazards + hazards;
			boolean b = MessageDialog.openQuestion(this.getTableContainer().getShell(), Messages.Confirm, confirmation);
			if (b) {
			...
			boolean b =
				MessageDialog.openQuestion(this.getTableContainer().getShell(), Messages.Confirm,
					Messages.PleaseConfirmTheDeletionOfAllSelectedHazards);
			if (b) {
				this.getDescriptionWidget().setText(""); //$NON-NLS-1$
				HazardsView.this.displayedHazard = null;
				for (Iterator<Hazard> i = selection.iterator(); i.hasNext();) {
			...
			if (event.getSelection() instanceof IStructuredSelection) {
				...
				if (selection.getFirstElement() instanceof Hazard) {
					...
					HazardsView.this.getDescriptionWidget().setText(
						((Hazard) selection.getFirstElement()).getDescription());
	...
	public boolean writeCSVData(BufferedCSVWriter writer) throws IOException {
		this.exportAsCSV(this.dataInterface.getAllHazards(),writer);
		return true;
	}
	
	@Override
	public Class<?> getExportWizard() {
		return HazardsWizard.class;
	}
}
