...
import astpa.wizards.stepData.AccidentsWizard;
...
public class AccidentsView extends CommonTableView {
	 ...
	 * 
	 */
	public static final String ID = "astpa.ui.acchaz.accidentsviewpart"; //$NON-NLS-1$
	...
	public void createPartControl(Composite parent) {
		...
		this.getFilterTextField().addKeyListener(new KeyAdapter() {
			...
			public void keyReleased(KeyEvent ke) {
				AccidentsView.this.getFilter().setSearchText(AccidentsView.this.getFilterTextField().getText());
		...
		Listener addAccidentListener = new Listener() {
			...
			public void handleEvent(Event event) {
				...
				AccidentsView.this.refreshView();
				AccidentsView.this.dataInterface.addAccident("", Messages.DescriptionOfThisAccident); //$NON-NLS-1$
				int newID = AccidentsView.this.dataInterface.getAllAccidents().size() - 1;
				AccidentsView.this.updateTable();
				AccidentsView.this.refreshView();
				AccidentsView.this.getTableViewer().setSelection(
					new StructuredSelection(AccidentsView.this.getTableViewer().getElementAt(newID)), true);
				AccidentsView.this.getTitleColumn().getViewer()
					.editElement(AccidentsView.this.getTableViewer().getElementAt(newID), 1);
			}
		};
		
		this.getAddNewItemButton().addListener(SWT.Selection, addAccidentListener);
		
		this.getTableViewer().getTable().addListener(SWT.KeyDown, addAccidentListener);
		...
		Listener returnListener = new Listener() {
			...
			public void handleEvent(Event event) {
				if ((event.type == SWT.KeyDown) && (event.keyCode == SWT.CR)
					&& (!AccidentsView.this.getTableViewer().getSelection().isEmpty())) {
					int indexFirstSelected = AccidentsView.this.getTableViewer().getTable().getSelectionIndices()[0];
					AccidentsView.this.getTitleColumn().getViewer()
						.editElement(AccidentsView.this.getTableViewer().getElementAt(indexFirstSelected), 1);
				}
			}
		};
		...
		this.getDescriptionWidget().addFocusListener(new FocusListener() {
			...
			public void focusGained(FocusEvent e) {
				...
				if (description.compareTo(Messages.DescriptionOfThisAccident) == 0) {
					...
					AccidentsView.this.dataInterface.setAccidentDescription(id, ""); //$NON-NLS-1$
		...
		this.getDescriptionWidget().addModifyListener(new ModifyListener() {
			...
			public void modifyText(ModifyEvent e) {
				if (AccidentsView.this.displayedAccident != null) {
					...
					String description = text.getText();
					UUID id = AccidentsView.this.displayedAccident.getId();
					AccidentsView.this.dataInterface.setAccidentDescription(id, description);
				}
			}
		});
		
		// Listener for showing the description of the selected accident
		AccidentsView.this.getTableViewer().addSelectionChangedListener(new AccSelectionChangedListener());
		...
		final EditingSupport titleEditingSupport = new AccEditingSupport(AccidentsView.this.getTableViewer());
		this.getTitleColumn().setEditingSupport(titleEditingSupport);
		
		TableViewerColumn linksColumn;
		linksColumn = new TableViewerColumn(AccidentsView.this.getTableViewer(), SWT.NONE);
		...
		linksColumn.setLabelProvider(new ColumnLabelProvider() {
			...
			public String getText(Object element) {
				...
				List<ITableModel> links =
					AccidentsView.this.dataInterface.getLinkedHazards(((Accident) element).getId());
				if (!(links == null)) {
		...
		// delete key
		AccidentsView.this.getTableViewer().getControl().addKeyListener(new KeyAdapter() {
			
			@Override
			public void keyReleased(final KeyEvent e) {
				if ((e.keyCode == SWT.DEL) || ((e.stateMask == SWT.COMMAND) && (e.keyCode == SWT.BS))) {
					IStructuredSelection selection =
						(IStructuredSelection) AccidentsView.this.getTableViewer().getSelection();
		...
		Menu menu = menuMgr.createContextMenu(AccidentsView.this.getTableViewer().getControl());
		menuMgr.addMenuListener(new IMenuListener() {
			
			@Override
			public void menuAboutToShow(IMenuManager manager) {
				if (AccidentsView.this.getTableViewer().getSelection().isEmpty()) {
					return;
				}
				if (AccidentsView.this.getTableViewer().getSelection() instanceof IStructuredSelection) {
					Action deleteAccident = new Action(Messages.Delete_accidents) {
	...
	public void deleteItems() {
		...
		IStructuredSelection selection = (IStructuredSelection) AccidentsView.this.getTableViewer().getSelection();
		if (selection.size() == 1) {
			// if only one accident is selected
			Accident deletedAccident = (Accident) selection.getFirstElement();
			String deletedAccidentId = Integer.toString(deletedAccident.getNumber());
			String deletedAccidentTitle = deletedAccident.getTitle();
			String confirmation = Messages.DoYouWishToDeleteTheAccident + deletedAccidentId + ": " //$NON-NLS-1$
			...
			String confirmation = Messages.PleaseConfirmTheDeletionOfTheFollowingAccidents + accidents;
			boolean b = MessageDialog.openQuestion(this.getTableContainer().getShell(), Messages.Confirm, confirmation);
			if (b) {
	...
	private class AccEditingSupport extends EditingSupport {
		...
		protected CellEditor getCellEditor(Object element) {
			return new TextCellEditor(AccidentsView.this.getTableViewer().getTable());
		}
		...
		protected Object getValue(Object element) {
			if (element instanceof Accident) {
				...
				if ((((Accident) element).getTitle().compareTo(Messages.DoubleClickToEditTitle)) == 0) {
		...
		protected void setValue(Object element, Object value) {
			if (element instanceof Accident) {
				...
				if (((Accident) element).getTitle().length() == 0) {
					((Accident) element).setTitle(Messages.DoubleClickToEditTitle);
				}
			}
			AccidentsView.this.refreshView();
		}
	...
	private class AccSelectionChangedListener implements ISelectionChangedListener {
		...
		public void selectionChanged(SelectionChangedEvent event) {
			...
			if (event.getSelection() instanceof IStructuredSelection) {
				...
				if (selection.getFirstElement() instanceof Accident) {
					if (AccidentsView.this.displayedAccident == null) {
						AccidentsView.this.getDescriptionWidget().setEnabled(true);
					} else {
						AccidentsView.this.displayedAccident = null;
					}
					AccidentsView.this.getDescriptionWidget().setText(
						((Accident) selection.getFirstElement()).getDescription());
					AccidentsView.this.displayedAccident = (Accident) selection.getFirstElement();
				}
			}
		}
	}
	...
	public void updateTable() {
		AccidentsView.this.getTableViewer().setInput(this.dataInterface.getAllAccidents());
	}
