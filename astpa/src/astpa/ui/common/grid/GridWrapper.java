...
public class GridWrapper {
	...
	public class NebulaGridRowWrapper extends GridItem {
		 ...
		 * @author Patrick Wickenhaeuser
		 * 
		 * @param parent the grid in which the row is added.
		 * @param style swt style.
		 * @param row the row to take the cells from.
		 * @param parentRow the parent of the row. Can be null if it doesn't
		 *            have a parent.
		 */
	...
	public static final Image getDeleteButton16() {
		if (GridWrapper.deleteLinkImage16 == null) {
			GridWrapper.deleteLinkImage16 =
				Activator.getImageDescriptor(GridWrapper.DELETE_LINK_ICON_PATH_16).createImage();
		}
	...
	public static final Image getLinkButton16() {
		if (GridWrapper.editLinkImage16 == null) {
			GridWrapper.editLinkImage16 =
				Activator.getImageDescriptor(GridWrapper.EDIT_LINK_ICON_PATH_16).createImage();
		}
	...
	public static final Image getLinkButton24() {
		if (GridWrapper.editLinkImage24 == null) {
			GridWrapper.editLinkImage24 =
				Activator.getImageDescriptor(GridWrapper.EDIT_LINK_ICON_PATH_24).createImage();
		}
	...
	public static final Image getDeleteButton32() {
		if (GridWrapper.deleteLinkImage32 == null) {
			GridWrapper.deleteLinkImage32 =
				Activator.getImageDescriptor(GridWrapper.DELETE_LINK_ICON_PATH_32).createImage();
		}
	...
	public static final Image getLinkButton32() {
		if (GridWrapper.editLinkImage32 == null) {
			GridWrapper.editLinkImage32 =
				Activator.getImageDescriptor(GridWrapper.EDIT_LINK_ICON_PATH_32).createImage();
		}
	 ...
	 * @param columnLabels the initial column labels. Can be empty.
	 */
	...
	private void resizeColumns() {
		int columnWidth = this.actualGrid.getSize().x / Math.max(1, GridWrapper.this.actualGrid.getColumnCount());
	 ...
	 * @author Patrick Wickenhaeuser
	 * 
	 * @param columnLabels the new labels.
	 */
	...
	public void fillTable() {
		...
		if (this.actualGrid.getColumnCount() > maxCellCount) {
			...
			for (int i = 0; i < (maxCellCount - this.actualGrid.getColumnCount()); i++) {
	...
	public boolean isCellSelected(IGridCell cell) {
		if (this.getGrid() != null) {
			...
			for (int i = 0; i < selections.length; i++) {
				IGridCell selectedCell = this.getGridCellFromCellCoordinate(selections[i]);
	 ...
	 * @author Patrick Wickenhaeuser
	 * 
	 * @param point the cell coordinate.
	 * @return the cell.
	 */
	 ...
	 * @author Patrick Wickenhaeuser
	 * 
	 * @param mousePoint the mouse coordinate.
	 * @return the cell.
	 */
	...
	public void activateCell(UUID uuid) {
		activateRecursive(this.rows, uuid);
	}
	 ...
	 * @author Benedikt Markt
	 * 
	 * @param cellRows the current grid row
			...
			for (IGridCell c : r.getCells()) {
				if (c.getUUID() != null) {
					if (c.getUUID().equals(uuid)) {
						LOGGER.info("Activating: " + uuid); //$NON-NLS-1$
						c.activate();
						return;
					}
				}
			}
			activateRecursive(r.getChildren(), uuid);
		}
	}
}