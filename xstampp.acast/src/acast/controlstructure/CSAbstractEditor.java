...
import acast.controlstructure.utilities.CSContextMenuProvider;
import acast.controlstructure.utilities.CSPalettePage;
import acast.controlstructure.utilities.CSPalettePreferences;
import acast.controlstructure.utilities.CSTemplateTransferDropTargetListener;
...
import acast.util.IZoomContributor;
 ...
 * 
 */
public abstract class CSAbstractEditor extends StandartEditorPart implements IControlStructureEditor, IZoomContributor {
	...
	public static Map<String, UUID> identifiers = new HashMap<>();
	...
	public void createPartControl(Composite parent) {
		...
		if (!alreadyStarted) {
			for (ICausalComponent x : this.modelInterface.getCasualComponents()) {
				comboList.add(x.getText());
				identifiers.put(x.getText(), x.getId());
			}
		}
		alreadyStarted = true;
		...
		data.right = new FormAttachment(CSAbstractEditor.FULL_SCALE);
		data.left = new FormAttachment(0);
		this.splitter = new FlyoutPaletteComposite(editorComposite, SWT.CENTER, this.getSite().getPage(),
	...
	protected void createGraphicalViewer(Composite parent) {
		...
		viewer.getControl().addFocusListener(new FocusListener() {
			...
			public void focusGained(FocusEvent e) {
				 ...
				 this.activation = contextService.activateContext("acastContextID"); //$NON-NLS-1$
			}
		});
	...
	public void configureGraphicalViewer(GraphicalViewer viewer) {
		...
		viewer.setEditPartFactory(new CSEditPartFactory(this.getModelInterface(), this.getId()));
		// zooming
		ScalableRootEditPart rootEditPart = new ScalableRootEditPart();

		viewer.setRootEditPart(rootEditPart);
		viewer.addDropTargetListener(new CSTemplateTransferDropTargetListener(viewer, this.getModelInterface()));
	...
	private void createToolBar(Composite parent, FormData toolbarData) {
		...
		preferenceButton.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				values.put("xstampp.command.preferencePage", "xstampp.acast.preferencepage.cs");
				STPAPluginUtils.executeParaCommand("acast.preferencepage", values);
			}
	 ...
	 * is an {@link UpdateAction}, it will have its <code>update() method
	 * called.
	 * 
	 * @param actions
	...
	public void mouseDoubleClick(MouseEvent e) {
		...
		if ((selection.size() == 1) && !(selection.get(0) instanceof CSConnectionEditPart)) {
			...
			CSAbstractEditPart selected = ((CSAbstractEditPart) selection.get(0));
			selected.performRequest(req);
//			try {
				ComponentType type = this.modelInterface.getComponent(selected.getId()).getComponentType();
				if (type.equals(ComponentType.ACTUATOR) || type.equals(ComponentType.CONTROLLER)
						|| type.equals(ComponentType.CONTROLLED_PROCESS) || type.equals(ComponentType.SENSOR) || type.equals(ComponentType.CONTROLACTION)) {
					IWorkbenchWindow window = PlatformUI.getWorkbench().getActiveWorkbenchWindow();
					IWorkbenchPage page = window.getActivePage();
					IWorkbenchPart part = window.getActivePage().getActivePart();

//					page.showView("A-CAST.view1");
					TableView.refresh(selected.getName(), false);;

					TableView.viewer.getTable().removeAll();
					TableView.updateReponsibilites(identifiers.get(TableView.combo.getText()),
							TableView.responsibility);
					page.activate(part);
				}
