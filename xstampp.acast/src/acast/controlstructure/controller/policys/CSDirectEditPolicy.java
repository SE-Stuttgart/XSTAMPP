...
import acast.controlstructure.controller.commands.ComponentRenameCommand;
...
public class CSDirectEditPolicy extends DirectEditPolicy {
	...
	protected Command getDirectEditCommand(DirectEditRequest request) {
		...
		ComponentRenameCommand command = new ComponentRenameCommand(this.oldName, this.dataModel, stepID);
		// The method getHost() calls the componentModel which makes the
		// requests

		Rectangle newLayout = ((DirectEditor) request.getCellEditor()).getBounds().getCopy();
		this.getHost().getFigure().translateToRelative(newLayout);
		command.setModel(this.model);
		command.setNewLayout(newLayout);
		if (CSAbstractEditor.identifiers.containsKey(this.value.replace("\n", ""))) {
			MessageBox dialog = new MessageBox(Display.getCurrent().getActiveShell(), SWT.ICON_INFORMATION | SWT.OK);
			dialog.setText("Warning");
			dialog.setMessage("Name already exists, please choose another one");

			dialog.open();
			command.setNewName(oldName);
		} else {
			command.setNewName(this.value.replace("\n", ""));
			CSAbstractEditor.identifiers.put(this.value.replace("\n", ""), CSAbstractEditor.identifiers.get(oldName));
			CSAbstractEditor.identifiers.remove(oldName);
		}
	...
	protected void showCurrentEditValue(DirectEditRequest request) {
		...
		if (!this.value.equals(this.oldName)) {
			((CSFigure) this.getHostFigure()).setForegroundColor(ColorConstants.white);
		}
