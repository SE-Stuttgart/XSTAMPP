...
public class CSConnectionModeProvider extends AbstractSourceProvider implements IPropertyChangeListener{
	...
	public Map getCurrentState() {
		...
		boolean f=Activator.getDefault().getPreferenceStore().getBoolean(IACASTPreferences.CONTROLSTRUCTURE_INDIVIDUAL_CONNECTIONS);
		if(f){
			map.put(CONNECTION_MODE, Boolean.TRUE);
		}else{
			map.put(CONNECTION_MODE, Boolean.FALSE);
		}
		f =Activator.getDefault().getPreferenceStore().getBoolean(IACASTPreferences.CONTROLSTRUCTURE_PROCESS_MODEL_BORDER); 
	...
	public void propertyChange(PropertyChangeEvent event) {
		if(event.getProperty().equals(IACASTPreferences.CONTROLSTRUCTURE_INDIVIDUAL_CONNECTIONS)){
		...
		}else if(event.getProperty().equals(IACASTPreferences.CONTROLSTRUCTURE_PROCESS_MODEL_BORDER)){
